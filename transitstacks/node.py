...
def define_node(component: pd.Series, verbose: bool = True) -> Node:
    ...
    exp_notify = False
    if component['End date'] and component['End date'] != np.NaN and str(component['End date']).lower() not in ["nan","nat","na","none"]:
        exp_notify = date.today() >= pd.to_datetime(component['End date']) - relativedelta(months=+EXPIRATION_NOTIFY_MONTHS)
    ...
    if verbose: print(f"NODE: {node_label}")
    ...
    if verbose: print(n.__dict__)
    if verbose: print(f"{node_class}: {n._icon} \n {n._icon_dir} \n {n._load_icon()}")
